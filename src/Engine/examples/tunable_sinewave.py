...
if __name__ == "__main__":
    ...
    flow.train([ [] , Engine.utils.ConcatenatingIterator([xtrain, freqtrain], ytrain) , []])
    #flow.train([ [] , [[mdp.numx.hstack((xtrain, freqtrain)), ytrain]], []])
                    
    # Save the states of the reservoir during training for later plotting
    flow.execute(Engine.utils.ConcatenatingIterator([xtrain, freqtrain]))
    ...
    ci = Engine.utils.ConcatenatingIterator([fb, freqtest])
