...
if __name__ == "__main__":
    ...
    [inputs, outputs] = Engine.datasets.analog_speech(indir="Lyon128")
    ...
    reservoir = Engine.nodes.SpikingIFReservoirNode(input_dim, output_dim=400, dtype=float, exc_frac=0.8, exc_w=0.1, inh_w= -0.5, input_w=0.2, cell_params=cell_params,
                                       syn_delay=1.0, Cprob_exc=0.2, Cprob_inh=0.2, Cprob_inp=0.2, kernel=Engine.nodes.exp_kernel(tau=30, dt=1), inp2spikes_conversion=poisson_gen(rngseed))
