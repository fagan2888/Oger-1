...
if __name__ == "__main__":
    ...
    readout = Engine.nodes.RidgeRegressionNode(0.0001, input_dim=N)
    readout2 = Engine.nodes.RidgeRegressionNode(0.0002, input_dim=N) 
    
    output_layers = mdp.hinet.SameInputLayer([readout, readout2])
    fb = Engine.nodes.FeedbackNode(n_timesteps=freerun_steps, input_dim=2)


    readout3 = Engine.nodes.RidgeRegressionNode(0.000001, input_dim=2)
    # build network with MDP framework
    flow = Engine.nodes.InspectableFlow([reservoir, output_layers, fb, readout3], verbose=1)
