...
class OdeNode(mdp.Node):
    ...
    def _execute(self, x):
        ...
        self.state = numpy.zeros((self._output_dim,),numpy.complex128)
        ...
        for n in range(steps):
            ...
            self.odeproblem.solve_DE(self.state,t1,t1+self.dt,dt_current,self.dt_min,self.dt_max)

            # Get the altered internal dt (useful to use that one to continue simulating in the next step i.o. init_dt)
            dt_current = self.odeproblem.get_timestep_after_simulation()

            self.states[n] = abs(self.state)

            if( int(1.0*n/steps*20.0) > puntjes ):
                print '.',
                puntjes = int(1.0*n/steps*20.0)
