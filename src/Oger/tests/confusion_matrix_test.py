...
import Oger
import numpy as np
from Oger.utils import confusion_matrix as conf
...
class ConfusionMatrixTest(unittest.TestCase):
    ...
    def test_constructor_labels(self):
        ...
        with self.assertRaises(RuntimeError) as cm:
            c = conf.ConfusionMatrix([[1,2],[3,4]], labels=[1,1])
    ...
    def test_add(self):
        c1 = conf.ConfusionMatrix([[1,2],[3,4]])
        c2 = conf.ConfusionMatrix([[3,4],[1,2]])
        s = c1 + c2
        t = conf.ConfusionMatrix([[4,6],[4,6]])
    ...
    def test_balance(self):
        c = conf.ConfusionMatrix([[1,3],[2,2]])
        cb = c.balance()
    ...
    def test_subsets(self):
        c = conf.ConfusionMatrix([[1,2,3],[4,5,6],[7,8,9]])
    ...
    def test_rates(self):
        c = conf.ConfusionMatrix([[8,2,1],[1,4,3],[1,2,7]])
    ...
    def test_binary(self):
        ...
        for b in bs:
            self.assertIsInstance(b, conf.BinaryConfusionMatrix)

    def test_from_data(self):
        with self.assertRaises(RuntimeError) as cm:
            i = np.atleast_2d(np.array([0, 1])).T
            t = np.atleast_2d(np.array([2])).T
            c = conf.ConfusionMatrix.from_data(3, i, t)
    ...
    def test_from_samples(self):
        ...
        c = conf.ConfusionMatrix.from_samples(3, i, t)
    ...
    def test_error_measure(self):
        ber = conf.ConfusionMatrix.error_measure('ber', 3)
        i = np.atleast_2d(np.array([0, 0, 1, 2, 0, 0, 2, 2, 1, 2])).T
        t = np.atleast_2d(np.array([0, 0, 0, 1, 1, 0, 2, 2, 1, 1])).T
        self.assertAlmostEqual(ber(i, t), 1.0/3)
        
    def test_sparse(self):
        c = conf.ConfusionMatrix([[5,0,0],[0,0,0],[1,0,3]])
...
class BinaryConfusionMatrixTest(unittest.TestCase):
    ...
    def test_constructor_dimension_checks(self):
        with self.assertRaises(RuntimeError) as cm:
            c = conf.BinaryConfusionMatrix([1,2,3])
        self.assertEqual(cm.exception.message, "ConfusionMatrix should be 2-dimensional - ndim is 1")
        
        with self.assertRaises(RuntimeError) as cm:
            c = conf.BinaryConfusionMatrix([[1,2,3],[4,5,6]])
        self.assertEqual(cm.exception.message, "ConfusionMatrix should have shape 2x2 - shape is 2x3")    
        
    def test_normalise(self):
        c = conf.BinaryConfusionMatrix([[1,2],[3,4]])
        cn = c.normalise()
        self.assertIsInstance(cn, conf.BinaryConfusionMatrix)
        self.assertTrue(np.all(cn.asarray() == np.array([[0.1,0.2],[0.3,0.4]])))
        
    def test_balance(self):
        c = conf.BinaryConfusionMatrix([[1,3],[2,2]])
        cb = c.balance()
        self.assertIsInstance(cb, conf.BinaryConfusionMatrix)
        self.assertTrue(np.all(cb.asarray() == np.array([[0.25,0.75],[0.5,0.5]])))

    def test_add(self):
        c1 = conf.BinaryConfusionMatrix([[1,2],[3,4]])
        c2 = conf.BinaryConfusionMatrix([[3,4],[1,2]])
        s = c1 + c2
        t = conf.BinaryConfusionMatrix([[4,6],[4,6]])
        self.assertIsInstance(s, conf.BinaryConfusionMatrix)
        self.assertTrue(np.all(s.asarray() == t.asarray()))
    ...
    def test_error_measure(self):
        ...
        recall = conf.BinaryConfusionMatrix.error_measure('recall')
