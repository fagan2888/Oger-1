...
NOTE: some updates (API change !) and documentation
      by Georg Holzmann, 2009
"""
...
class MFCC(object):
    def __init__(self, nfilt=40, ncep=13,
                 ...
                 samprate=16000, frate=100, winsize=256,
                 nfft=512):
        """ Initializes the MFCC extraction object.
        
        nfile    -- Number of filters to use for creating the mel-scale.
        ncep     -- Number of cepstral coefficients to compute.
        lowerf   -- Low end of filter band.
        upperf   -- High end of filter band.
        alpha    -- Scaling factor for pre-emphasis of input audio data.
        samprate -- Sampling rate at which the audio data to be processed
                    was captured, specified in samples per second.
        frate    -- Number of frames of data to be processed per second of sampled audio.
        winsize  -- Size of the Analysis FFT window, in number of samples.
        nfft     -- FFT size for mel filters.
        """
...
#            print self.filters[leftfr:rightfr,whichfilt]
    ...
    def sig2s2mfc(self, sig):
        """ Calculates MFCCs of the signal vector.
        """
    ...
    def sig2logspec(self, sig):
        """ Calculates the filter-weighted log spectrum of the signal vector.
        """
        nfr = int(len(sig) / self.fshift + 1)
    ...
    def frame2logspec(self, frame):
        ...
        # Slaney uses log10 here !
#        return numpy.log10(numpy.dot(power, self.filters).clip(1e-5,numpy.inf))
